...
bool CSi4463::initialize()
{
    ...
    if( rc == rcSuccess )
    {
        MYDEBUG("si init OK");
        byte cmd[] = {0x23, 0x00};
        SiResultCode stat = execCmdChain(cmd);
    ...
    byte packet[] = { 0x23,0x00 };
    rc = execCommand(2, packet, 0, NULL);
...
CSi4463::SiResultCode CSi4463::execCommand(size_t numBytesToWrite, const byte* writeData, size_t numBytesToRead, byte* readData)
{
    ...
    {
        Serial.println(writeByte(writeData[i]),HEX);
    ...
    {
        Serial.println(writeByte(0),HEX);
    }
    endCommunication();
